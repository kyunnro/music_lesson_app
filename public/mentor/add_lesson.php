...
if (!$course_id) {
    header("Location: courses.php?error=No course specified for adding a lesson.");
    exit();
}
...
try {
    ...
    error_log("Error fetching course for adding lesson: " . $e->getMessage());
    header("Location: courses.php?error=Database error loading course.");
    exit();
}
...
<head>
    ...
    <title>Add New Lesson - Mentor Panel</title>
...
<body>
    <div class="container wide-container">
        ...
        <h1><i class="fas fa-plus-circle icon"></i>Add New Lesson for "<?php echo htmlspecialchars($course['title']); ?>"</h1>
        <p>Fill in the details below to add a new lesson to this course.</p>
        ...
        <form action="../api/mentor/process_lesson.php" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="url" id="video_url" name="video_url" class="form-control" placeholder="e.g., https://youtube.com/watch?v=..." required>
            </div>
            <div class="form-group">
                <label for="materials_url"><i class="fas fa-file-alt icon"></i>Materials URL (Optional)</label>
                <input type="url" id="materials_url" name="materials_url" class="form-control" placeholder="e.g., https://example.com/materials.pdf">
            </div>
            <div class="form-group">
                <label for="quiz_data"><i class="fas fa-question-circle icon"></i>Quiz Data (JSON Format, Optional)</label>
                <textarea id="quiz_data" name="quiz_data" class="form-control" rows="8" placeholder='<?php echo htmlspecialchars($quiz_example); ?>'></textarea>
            ...
            <div class="form-group">
                ...
                <input type="number" id="lesson_order" name="lesson_order" class="form-control" min="1" value="1" required>
            </div>
            <button type="submit" class="btn btn-block"><i class="fas fa-save icon"></i>Save Lesson</button>
        </form>
